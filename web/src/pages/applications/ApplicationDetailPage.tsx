...
export const ApplicationDetailPage: React.FC = () => {
  ...
  if (error || !app) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-12">
        <ErrorMessage message={error || 'Application not found'} onRetry={() => id && loadApp(id)} />
        <div className="mt-4">
  ...
  const statusSteps = [
    {
      ...
      date: app.submitted_at ? new Date(app.submitted_at).toLocaleString() : 'Pending',
      done: true,
      desc: 'Owner submitted statutory verification request with instrument details.',
    },
    {
      title: 'Jurisdiction Assignment',
      date: app.assignment?.assigned_at ? new Date(app.assignment.assigned_at).toLocaleString() : 'Pending',
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-gov flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <span className="text-[11px] font-mono font-bold text-pramaan-gold-700 uppercase tracking-wider">
            {app.application_type === 'RE_VERIFICATION' ? 'Statutory Annual Re-Verification' : 'Initial Stamping Verification'}
          </span>
          <h1 className="text-2xl font-mono font-bold text-slate-900 mt-0.5">{app.application_number}</h1>
          <p className="text-xs text-slate-500 mt-1">
            Applicant: <strong>{app.applicant_name}</strong> ({app.applicant_organization})
          </p>
        </div>
      ...
      <Card title="Statutory Verification Progress Timeline" subtitle="End-to-end processing milestones">
        <div className="py-4">
              ...
              <div key={idx} className="relative">
                ...
                <div>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                    <h4 className={`text-sm font-semibold ${step.done ? 'text-slate-900' : 'text-slate-400'}`}>
                      {step.title}
                    </h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Instrument Card */}
        <Card title="Target Instrument Under Test" subtitle="Declared device specifications">
          <dl className="space-y-3 text-xs">
            <div className="flex justify-between border-b border-slate-100 pb-2">
              <dt className="text-slate-500">Instrument Name:</dt>
              <dd className="font-semibold text-slate-900">{app.instrument_name}</dd>
            </div>
            <div className="flex justify-between border-b border-slate-100 pb-2">
              <dt className="text-slate-500">Serial Number:</dt>
              <dd className="font-mono font-bold text-slate-800">{app.instrument_serial}</dd>
            </div>
            <div className="flex justify-between border-b border-slate-100 pb-2">
              <dt className="text-slate-500">Classification Type:</dt>
              <dd className="text-slate-800 text-right">{app.instrument_type_name}</dd>
            </div>
            <div className="pt-2">
              <Link to={`/instruments/${app.instrument_id}`} className="text-xs font-semibold text-pramaan-navy-800 hover:underline">
        ...
        {/* Application Details */}
        <Card title="Statutory Remarks & Purpose" subtitle="Applicant submission notes">
          <div className="space-y-4 text-xs">
            <div>
              <span className="text-slate-500 font-semibold block mb-1">Declared Purpose:</span>
            ...
            {app.remarks && (
              <div>
                <span className="text-slate-500 font-semibold block mb-1">Additional Remarks:</span>
