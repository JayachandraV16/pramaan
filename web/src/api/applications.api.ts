...
export function mapBackendApplicationToFrontend(item: any): VerificationApplication {
  return {
    ...
    instrument_name: item.instrument_name || item.instrumentName || 'Weighing Instrument',
    instrument_serial: item.serial_number || item.instrument_serial || item.serialNumber || 'SN-N/A',
    instrument_type_name: item.instrument_type_name || item.instrumentTypeName || 'Metrological Asset',
