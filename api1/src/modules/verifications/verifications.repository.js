...
async function findScheduleById(id) {
  const { rows } = await pool.query(
    `SELECT id, application_id, assignment_id,
...
// Find active schedule for an assignment
async function findActiveScheduleByAssignmentId(assignmentId) {
  const { rows } = await pool.query(
    `SELECT id, application_id, assignment_id,
            TO_CHAR(scheduled_date, 'YYYY-MM-DD') AS scheduled_date,
            scheduled_time::text AS scheduled_time,
...
// Find the latest verification for an assignment.
// Prevents an officer from starting a second verification for the same assignment.
async function findLatestVerificationByAssignmentId(assignmentId) {
  const { rows } = await pool.query(
    `SELECT v.*,
            vr.decision,
            vr.remarks AS result_remarks,
            vr.result_date
...
// Mark the assignment as completed after a final PASS/FAIL result.
...
// Mark the linked schedule as completed after a final PASS/FAIL result.
...
async function findAllVerifications() {
  const { rows } = await pool.query(
    `SELECT v.*,
     ...
     LEFT JOIN verification_results vr
       ON vr.verification_id = v.id
     LEFT JOIN verification_certificates vc
       ON vc.verification_id = v.id
     ORDER BY v.created_at DESC`
  );
...
async function completeApplication(applicationId, decision = 'PASS') {
  const status = decision === 'FAIL' ? 'REJECTED' : 'COMPLETED';
...
module.exports = {
  findAssignmentById,
  findScheduleById,
  findActiveScheduleByAssignmentId,
  findLatestVerificationByAssignmentId,
  createVerification,
  createObservation,
  createReading,
