...
async function createSchedule(user, data) {
  ...
  // Cannot schedule applications that are rejected, cancelled, or completed
  const applicationsRepo = require('../applications/applications.repository');
  const app = await applicationsRepo.findApplicationById(data.applicationId);
  if (app && (app.status === 'REJECTED' || app.status === 'CANCELLED' || app.status === 'COMPLETED')) {
    throw ApiError.badRequest(`Cannot schedule an application with status ${app.status}`);
  }
  ...
  // Notify Applicant and Officer
  try {
    const notificationsRepo = require('../notifications/notifications.repository');
    if (assignment.applicant_id) {
      await notificationsRepo.createNotification({
        recipientId: assignment.applicant_id,
        type: 'VERIFICATION_SCHEDULED',
        title: 'Verification Scheduled',
        message: `Verification for application ${assignment.application_number} has been scheduled for ${data.scheduledDate} at ${data.scheduledTime}.`,
        relatedApplicationId: data.applicationId,
      });
    }
    if (assignment.assigned_to_id) {
      await notificationsRepo.createNotification({
        recipientId: assignment.assigned_to_id,
        type: 'VERIFICATION_SCHEDULED',
        title: 'Verification Scheduled',
        message: `Verification for application ${assignment.application_number} has been scheduled for ${data.scheduledDate} at ${data.scheduledTime}.`,
        relatedApplicationId: data.applicationId,
